...
- [Helm values inheritance and environment overrides](helm/docs/helm-values-inheritance.md) — how Helm values layer across dev, staging, and prod for microservice releases
- [Helm release workflow script](helm/scripts/helm-release-workflow.sh) — lint, template, diff, upgrade, and rollback in one runnable script
- [Tag-driven release branches and conventional-commit gates](git/docs/git-cicd-tag-driven-releases-conventional-commits.md) — two complementary patterns for wiring Git tags into CI/CD release workflows
- [Git repo scaffold template](git/templates/repo-scaffold/README.md) — copy-in layout with commit hooks, branch protection, CODEOWNERS, and CONTRIBUTING
- [Multi-arch buildx automation](docker/scripts/image-build-automation.sh) — Docker buildx script for multi-platform images with registry caching and semver tagging
