...
- docker: Add multi-service Compose app scaffold with health-gated startup, file-based secrets, and tools profile (docker-016)
...
- ansible: Add gated ansible-playbook wrapper with syntax-check, check-diff, idempotency rerun, and recap parsing (ansible-014)
...
- grafana: Add Loki datasource provisioning YAML config (grafana-006 rework)
...
- docker: Add multi-service Docker Compose manifest with web app, cache, and worker (docker-008)
...
- bash: Rework debug and profiling docs — fix L3 voice, remove unverified citation, reduce overlap with strict-mode doc (bash-013)
