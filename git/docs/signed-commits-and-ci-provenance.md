...
This document covers how to make Git commits verifiable and how to connect that verifiability to CI/CD pipelines. It addresses three related concerns: signing commits with GPG or SSH keys, protecting pushes with branch rules and push protection, and producing build attestation that links a published artifact back to the exact source commit, the build environment, and the CI run that produced it.
...
- A recent Git with `gpg.format ssh` support for SSH commit signing.
...
If a signed commit is found to be compromised or erroneous:
- Revert the commit with `git revert <sha>` — this creates a new commit that undoes the changes and can also be signed.
- If the signing key itself is compromised, revoke the key on the Git host, generate a new key pair, update `user.signingkey`, and re-publish the public key.
- For attestation: if an artifact's attestation is invalid or missing, do not promote the artifact. Rebuild from a verified commit and regenerate the attestation.
...
| SSH signing configured but `gpg.format ssh` has no effect | Git version lacks SSH signing support. | Upgrade to a recent Git release that includes `gpg.format ssh`. |

## References

- `git commit --help` — `-S`, `--gpg-sign` options and signing configuration.
- `git verify-commit --help` — verification command reference.
- `git config --help` — `user.signingkey`, `gpg.format`, `gpg.ssh.allowedSignersFile` settings.
- Git host documentation (GitHub, GitLab, Bitbucket) — branch protection rules, push protection, and required signed commits settings.
- SLSA Provenance specification — attestation format and verification model.